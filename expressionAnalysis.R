...
# Make a Tree Map ####
StatsFinal <-  rnaseqProject$metaDataDF %>% group_by_(.dots= c("DIAGNOSIS","DIAGNOSIS.Alias.TreeMap", "DIAGNOSIS.substatus", rnaseqProject$factorName, "Color", "LIBRARY_TYPE.TreeMap") ) %>% 
  ...
  data.frame() %>% distinct(DIAGNOSIS,DIAGNOSIS.Alias.TreeMap,SampleSum, .keep_all = TRUE)

StatsFinal <- StatsFinal %>% dplyr::filter(DIAGNOSIS.Alias != "NS")
StatsFinal[,"LegendSampleSum"] <- paste(StatsFinal[,"DIAGNOSIS.Alias.TreeMap"],"( ",StatsFinal[,"Sample"], " )",sep="")
StatsFinal

## Make the plot
pdf(file=paste(
    paste(rnaseqProject$workDir,rnaseqProject$projectName, rnaseqProject$plotsDir,"Histology_Tree_Map_All", sep = "/"),
    "pdf",sep="."), height=8, width= 10)

ggplot(StatsFinal, aes(area = Sample, fill = Color, label=DIAGNOSIS.Alias.TreeMap, subgroup=DIAGNOSIS)) +
...
### Read DataSets ####
mergeObjectsNoDup_data <- readRDS(paste(rnaseqProject$workDir,rnaseqProject$projectName,rnaseqProject$outputdirRDSDir,"RawCount","mergeObjectsNoDup.rawcounts.2020-01-31.rds",sep="/"))
...
design %<>% arrange( LIBRARY_TYPE,DIAGNOSIS )
print("design")
print(dim(design))
...
AliasNames_df                 <- dplyr::left_join( data.frame("Sample.Biowulf.ID.GeneExp"=colnames(expressionTMM.RPKM.arr)), 
                                                   designMatrix[,c(rnaseqProject$metadataFileRefCol,rnaseqProject$factorName,"Sample.ID.Alias", 
                                                                   "Sample.Data.ID", "DIAGNOSIS.Alias", "Database.name")] )
...
## Make plot 
pdf(file=paste(
  paste(rnaseqProject$workDir,rnaseqProject$projectName, rnaseqProject$plotsDir,"hirarchial_clustering_nofrozen", sep = "/"),
...
ssGSEAScores            <- corUtilsFuncs$parseBroadGTCOutFile("../Glioma.data/GSEA/output/RPKM_Data_Filt_Consolidated.GeneNames.log2.ssGSEA2020-01-31.PROJ.gct")
...
selected.metadata              <- rnaseqProject$validMetaDataDF  %>% 
  ...
  dplyr::select_( .dots=c(rnaseqProject$metadataFileRefCol, rnaseqProject$factorName, "Database.name", "Mutation.Generic" ) )
...
Scores <- cbind(t(ssGSEAScores.HLA.Cyto.Selected), selected.metadata[,"Mutation.Generic", drop=FALSE]) %>% 
  dplyr::rename_(.dots = setNames( list("Mutation.Generic"), list("Diagnosis") )) #%>%
...
customColorDF <- data.frame(Color=unique(as.character(rnaseqProject$metaDataDF$Mutation.Generic.color)) , 
                                 Diagnosis= unique(as.character(rnaseqProject$metaDataDF$Mutation.Generic)))
customColorDF
...
SBName                <- paste(rnaseqProject$workDir, rnaseqProject$projectName, rnaseqProject$plotsDir,
                               "TMM-RPKM.ssGSEA.enrichmentScores.byVariantProfile.bean.log.pdf",sep="/")
ggsave(SBName, marrangeGrob(EnrischmentScorePlots,ncol=2,nrow=1 ), width = 10, height = 8)
