rm(list = ls())
workDir = "T:/Sivasish_Sindiri/R Scribble/Glioma.data/"
...
makResultMatrix <- function(x) {
  ...
  data <- tryCatch({
    ...
    read.csv(paste0("./",x),sep="\t", header = TRUE, stringsAsFactors = FALSE )
...
fusionFiles <- list.files("./"); length(fusionFiles)
...
### Step  Remove cell lines
fusionFile.NoCL <- fusionFile %>% dplyr::filter(!grepl("CellLine", LIBRARY_TYPE)) %>% data.table(); dim(fusionFile.NoCL) 

### Step 1 decondensing
newCols <- c("SPTool1", "SPTool2", "SPTool3")
fusionFileSplits.v1 <- fusionFile.NoCL %>% tidyr::separate( spanreadcount, newCols , " " )
fusionFileSplits.v1[is.na(fusionFileSplits.v1)] <- 0 
fusionFileSplits.v1[,(newCols):= lapply(.SD, as.numeric), .SDcols = newCols]
## Sanity Check
...
neoepitopesFromFusionsID.Filtered.Final <- neoepitopesFromFusionsID.AddOne[primaryID %in% validFusionsForNeoantigenID$primaryID.AddOne]; 
dim(neoepitopesFromFusionsID.Filtered.Final)
neoepitopesFromFusionsID.NotFound.Filtered.Final <- neoepitopesFromFusionsID.AddOne[!(primaryID %in% validFusionsForNeoantigenID$primaryID.AddOne)]; 
dim(neoepitopesFromFusionsID.NotFound.Filtered.Final)
##

################### Sanity Check #################################
samplesFromValidFusions <- unique(validFusionsForNeoantigenID$Sample.ID.Alias)
SamplesFromNeoantigen   <- unique(neoepitopesFromFusionsID.AddOne$Sample.ID.Alias)
SamplesFromFinalNeoantigen <- unique(neoepitopesFromFusionsID.Filtered.Final$Sample.ID.Alias)
print(paste(
      paste0("Samples in validFusionsForNeoantigenID ", length(samplesFromValidFusions)),
      paste0("Samples in neoepitopesFromFusionsID.AddOne ", length(SamplesFromNeoantigen)),
      paste0("Samples in neoepitopesFromFusionsID.Filtered.Final ", length(SamplesFromFinalNeoantigen)) 
      ))

samplesInFinalNeo <- SamplesFromFinalNeoantigen[ which(SamplesFromFinalNeoantigen %in% samplesFromValidFusions )]
paste("Samples match in validFusionsForNeoantigenID & neoepitopesFromFusionsID.Filtered.Final", length(samplesInFinalNeo))
samplesNotInFinalFusion <- SamplesFromFinalNeoantigen[which(!SamplesFromFinalNeoantigen %in% samplesFromValidFusions )]
paste("Samples match in neoepitopesFromFusionsID.Filtered.Final & not in validFusionsForNeoantigenID", length(samplesNotInFinalFusion))
samplesNotInNeoFinal <- samplesFromValidFusions[which(!samplesFromValidFusions %in% SamplesFromFinalNeoantigen)]
paste("Samples match in neoepitopesFromFusionsID.Filtered.Final & not in validFusionsForNeoantigenID", length(samplesNotInNeoFinal))
#################################################################

write.table(neoepitopesFromFusionsID.Filtered.Final, paste0(workDir,"/NoMaxEffinityEpitope.Filtered.Normal.ReadThrough.GTE.3.ValidFusions.txt"), sep="\t", row.names = FALSE, quote = FALSE)
write.table(neoepitopesFromFusionsID.NotFound.Filtered.Final, paste0(workDir,"/.NotFound.NoMaxEffinityEpitope.Filtered.Normal.ReadThrough.GTE.3.ValidFusions.txt"), sep="\t", row.names = FALSE, quote = FALSE)



## Count NeoEpitopes per sample
EpitopesGroupBySample <- neoepitopesFromFusionsID.Filtered.Final %>% dplyr::group_by(Sample.Data.ID ) %>% mutate(FusionNeoAntigenCount = n()) %>% 
...
write.table(EpitopesGroupBySample.ToPrint, paste0(workDir,"/EpitopesGroupBySampleFilt.ReadThrough.GTE.3.noMaxAffinityHLA.ValidFusions.txt"), sep="\t", row.names = FALSE, quote = FALSE)
